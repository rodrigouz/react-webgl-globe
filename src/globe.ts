...
DAT.Globe = function(container, opts) {
  ...
  function init() {
    ...
    container.style.width = `${opts.w}px`;
    container.style.height = `${opts.h}px`;
  ...
  this.__defineGetter__('time', () => {
    return this._time || 0;
  });

  this.__defineSetter__('time', t => {
    var validMorphs = [];
    var morphDict = this.points.morphTargetDictionary;
    for (var k in morphDict) {
      if (k.indexOf('morphPadding') < 0) {
        validMorphs.push(morphDict[k] as never);
      }
    }
    validMorphs.sort();
    ...
    if (lastIndex >= 0 && this.points.morphTargetInfluences) {
